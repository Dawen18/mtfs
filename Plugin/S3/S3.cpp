...
pluginSystem::S3::S3() : nextFreeInode(1), nextFreeDirectory(0), nextFreeData(0) {
	this->freeDatas.clear();
	this->freeDirectory.clear();
	this->freeInodes.clear();
}

std::string pluginSystem::S3::getName() {
	return S3::NAME;
}

bool pluginSystem::S3::attach(std::map<std::string, std::string> params) {
...
int pluginSystem::S3::add(uint64_t *id, const mtfs::blockType &type) {
...
int pluginSystem::S3::del(const uint64_t &id, const mtfs::blockType &type) {
	mutex *mu = nullptr;
	vector<uint64_t> *ve = nullptr;
	uint64_t *nextId;
...
int pluginSystem::S3::get(const uint64_t &id, void *data, const mtfs::blockType &type, bool metas) {
...
int pluginSystem::S3::put(const uint64_t &id, const void *data, const mtfs::blockType &type, bool metas) {
	string filename = this->home, key;
	switch (type) {
		case mtfs::INODE:
			filename += "in";
...
bool pluginSystem::S3::getSuperblock(mtfs::superblock_t &superblock) {
	return false;
}

bool pluginSystem::S3::putSuperblock(const mtfs::superblock_t &superblock) {
	return false;
}

int pluginSystem::S3::writeTmpFile(const string &filename, const void *data, const mtfs::blockType &type) {
	switch (type) {
		case mtfs::INODE:
...
int pluginSystem::S3::writeInode(const std::string &filename, const mtfs::inode_t *inode) {
	Document d;
	d.SetObject();
...
int pluginSystem::S3::writeDirBlock(const std::string &filename, const mtfs::dirBlock_t *dirBlock) {
	Document d;
	d.SetObject();
...
int pluginSystem::S3::writeDataBlock(const std::string &filename, const uint8_t *datas) {
	ofstream file(filename);
	if (!file.is_open())
		return errno != 0 ? errno : ENOENT;
...
int pluginSystem::S3::writeSuperblock(const std::string &filename, const mtfs::superblock_t *superblock) {
	ofstream sbFile(filename, ios::binary);
	sbFile.write((const char *) superblock, sizeof(*superblock));
	sbFile.close();
}

int pluginSystem::S3::dlObj(const std::string &filename, const std::string &key) {
...
int pluginSystem::S3::upObj(const std::string &filename, const std::string &key) {
...
bool pluginSystem::S3::dirExists(string path) {
	struct stat info{};
...
void pluginSystem::S3::writeMetas() {
	Document d;
	d.SetObject();
...
void pluginSystem::S3::delObj(const string &key) {
	...
	if (deleteObjectOutcome.IsSuccess()) {
		...
		string message = string("PutObject error: ") + deleteObjectOutcome.GetError().GetExceptionName().c_str() + " " +
